...
import { useChatHubConnection, ChatEventHandlers, sameId } from "../singlr/ChatHubConnection";
...
export function useChatHub({
  ...
  const { sendTyping, sendStopTyping, registerHandlers, activeChatUserIdRef } = useChatHubConnection();
  ...
  useEffect(() => {
    handlersRef.current = { onTyping, onStopTyping, onNewMessage, onUserOnline, onUserOffline };
  });
  ...
  useEffect(() => {
    const stableHandlers: ChatEventHandlers = {
      ...
      onUserOffline: (uid) => handlersRef.current.onUserOffline?.(uid),
    };
