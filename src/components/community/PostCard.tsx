...
export default function PostCard({ post, currentUserId }: { post: mainPostData; currentUserId: string }) {
  ...
  const { mutate: doDelete, isPending: isDeleting } = useDeletePost(currentUserId);
  ...
  const goToProfile = (e: React.MouseEvent) => {
    e.stopPropagation();
    navigate(isOwner ? "/community/profile/me" : `/community/profile/${post.userId}`);
  };
    ...
    <>
      <article className="flex gap-3 px-4 py-3 hover:bg-white/[0.03] transition-colors border-b border-[#2f3336] cursor-pointer group">
        <button className="flex-shrink-0 self-start" onClick={goToProfile}>
          <img src={post.profilePhoto || "/assets/images/default-avatar.png"} alt={post.userName} className="w-10 h-10 rounded-full object-cover hover:opacity-90 transition" />
        </button>

        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <div className="flex items-center gap-1 flex-wrap min-w-0">
              <button onClick={goToProfile} className="text-white font-bold text-[15px] hover:underline truncate">{post.userName}</button>
            ...
            <div className={`mt-3 rounded-2xl overflow-hidden grid gap-0.5 border border-[#2f3336] ${post.files.length === 1 ? "grid-cols-1" : "grid-cols-2"}`}>
              {post.files.slice(0, 4).map((file, idx) => (
                <div key={idx} className="relative overflow-hidden">
                  <img src={file} className="w-full h-52 object-cover hover:opacity-90 transition" alt="" />
